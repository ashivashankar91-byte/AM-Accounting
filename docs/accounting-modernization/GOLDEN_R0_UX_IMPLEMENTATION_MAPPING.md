...
| 11 | Error codes | `400 MISSING_ENTITY`; `500 UNCLASSIFIED_ACCOUNT_TYPE {error, accounts[]}`. **`STRUCTURAL_IMBALANCE` is never thrown by `getIncomeStatement()`** — that check exists only in `getBalanceSheet()`. COST_OF_SALES/DISTRIBUTION accounts are excluded as diagnostics with `netIncome` unaffected, no error. This narrows `KNOWN_LIMITATIONS_REGISTER.md` row 3's wording, which is accurate for BS but doesn't apply to IS the same way. |
...
| UXMAP-22 | `KNOWN_LIMITATIONS_REGISTER.md` row 3 wording conflates BS-specific `STRUCTURAL_IMBALANCE` with Income Statement, where only `UNCLASSIFIED_ACCOUNT_TYPE` applies | S227 (IS) | PRODUCT_DECISION_REQUIRED |
