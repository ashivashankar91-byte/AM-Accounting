import { Prisma } from '.prisma/fixedops-client';
import { RlsTenantContext, setTenantContextOnConnection } from '@amacc/shared-kernel';
import pino from 'pino';

const logger = pino({ name: 'serializable-retry' });

const MAX_RETRIES = 5;
const BASE_DELAY_MS = 100;

/** Serializable-isolation transaction retry with tenant-context propagation.
 * Every money-guard transaction in this service (RO close, reversal,
 * distribution writes) runs under this so concurrent duplicate submits
 * resolve to one winner without a second locking mechanism. */
export async function withSerializableRetry<T>(
  prisma: any,
  fn: (tx: Prisma.TransactionClient) => Promise<T>,
): Promise<T> {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        await setTenantContextOnConnection(tx as any, RlsTenantContext.get());
        return fn(tx);
      }, {
        isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
      });
    } catch (err: any) {
      const isSerializationFailure =
        err.code === 'P2034' ||
        err.message?.includes('could not serialize access') ||
        err.meta?.code === '40001';

      if (isSerializationFailure && attempt < MAX_RETRIES) {
        const delay = BASE_DELAY_MS * Math.pow(2, attempt - 1);
        logger.warn({ attempt, delay, error: err.message }, 'Serialization failure; retrying');
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue;
      }
      throw err;
    }
  }
  throw new Error('Serialization retry exhausted after 5 attempts');
}
