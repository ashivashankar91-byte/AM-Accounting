#!/usr/bin/env bash
# CE-11 fixedops-service — repeatable ephemeral Postgres for live-database /
# RLS integration tests, scoped to this service only. Mirrors
# services/tax-service/tests/live-db/setup.sh exactly (same scratch/socket
# layout, same trust-auth ephemeral cluster approach). NEVER touches the
# shared `amacc` dev database: this spins up a completely separate,
# disposable cluster on its own port/socket dir.
#
# Usage:
...
PGPORT="${AMACC_FIXEDOPS_TEST_PORT:-55470}"
DB_NAME="amacc_fixedops_service_test"
...
echo "==> applying fixedops-service migrations, in order"
for f in \
  "$SVC_ROOT/prisma/migrations/20260801040000_init_fixedops_service/migration.sql" \
  "$SVC_ROOT/prisma/migrations/20260801040001_add_rls_policies_fixedops_svc/migration.sql" \
  "$SVC_ROOT/prisma/migrations/20260802010000_s063_labor_rate_policy/migration.sql" \
  "$SVC_ROOT/prisma/migrations/20260802010001_add_rls_policies_labor_rate/migration.sql" \
  "$SVC_ROOT/prisma/migrations/20260802010002_s063_tech_guarantee_config_governance/migration.sql"; do
