...
export async function docRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request, reply) => {
    ...
    const { legalEntityId, asOfDate } = request.query as any;
    return reply.send(await container.resolve(DocService).getDoc(getTenantId(request), legalEntityId, asOfDate));
  });
}
