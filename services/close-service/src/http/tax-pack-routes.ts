...
export async function taxPackRoutes(app: FastifyInstance) {
  ...
  app.post('/generate', async (request, reply) => {
    ...
    return reply.code(201).send(await container.resolve(TaxPackService).generate(tenantId, body.legalEntityId, parseInt(body.fiscalYear), actor));
  });
}
