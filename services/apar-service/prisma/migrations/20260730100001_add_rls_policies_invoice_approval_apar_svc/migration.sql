-- S041: Row Level Security for the invoice-approval tables, following the
-- exact pattern shipped for S036A/S038/S046/S039 (see
-- 20260729010001_add_rls_policies_apar_svc for the amacc_rls_bypass
-- rationale, referenced here rather than repeated).
--
-- ap_invoice_approval_steps is a child table without its own tenant_id
-- column — RLS is enforced via its parent ap_invoice_approval_instances,
-- same convention as vendor_invoice_lines/po_lines.

DO $$
DECLARE
  t TEXT;
  tables TEXT[] := ARRAY[
    'ap_invoice_approval_rules', 'ap_invoice_approval_instances'
  ];
BEGIN
