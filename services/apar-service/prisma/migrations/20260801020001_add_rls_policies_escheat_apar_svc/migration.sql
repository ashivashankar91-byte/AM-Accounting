-- CE-09 S045: Row Level Security for the void/stop/reissue/escheat tables,
-- following the exact pattern shipped for S036A/S038/S039/S041/S043A/S042.
-- ap_manual_payments already has RLS from the S043A migration; the new
-- columns added there (cleared_at, cleared_by, reissue_of_payment_id,
-- issue_date) are covered by that table's existing policies.

DO $$
DECLARE
  t TEXT;
  tables TEXT[] := ARRAY[
    'ap_stop_payment_requests', 'ap_escheat_jurisdiction_configs',
    'ap_escheat_due_diligence_records', 'ap_escheat_transfers', 'ap_escheat_gl_account_configs'
  ];
BEGIN
