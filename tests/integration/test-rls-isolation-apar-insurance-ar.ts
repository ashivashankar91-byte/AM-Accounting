/**
 * CE-09 S049 — PostgreSQL Row Level Security isolation proof for the five
 * new insurance-AR tables, against a real Postgres instance. Modeled
 ...
 * technique for child tables that carry a real FK to their parent claim
 * row.
 *
 * Prerequisites: tests/integration/rls-live-db/setup.sh (applies
 * apar-service's full migration history, including S049's migrations,
 ...
 *   npx tsx test-rls-isolation-apar-insurance-ar.ts
 */
import pg from 'pg';
...
async function testTable(spec: TableSpec) {
  const tenantA = `apar-rls-ia-a-${randomUUID()}`;
  const tenantB = `apar-rls-ia-b-${randomUUID()}`;
...
async function main() {
  await testTable({
    table: 'ar_insurance_claims',
    insertSql: `INSERT INTO ar_insurance_claims (id, tenant_id, customer_id, insurer_name, claim_number, claim_amount, created_by)
                VALUES ($1, $2, 'customer-rls-test', 'RLS Test Insurance Co', 'CLM-RLS-TEST', 1000.00, 'rls-test-user')`,
    insertParams: (id, tenant) => [id, tenant],
    updateSql: `UPDATE ar_insurance_claims SET status = 'CLOSED' WHERE id = $1`,
    cleanupTables: ['ar_insurance_claim_supplements', 'ar_insurance_payment_applications', 'ar_insurance_short_pay_dispositions', 'ar_insurance_claims'],
  });

  await testTable({
    table: 'ar_insurance_claim_supplements',
    insertSql: `WITH claim AS (
                  INSERT INTO ar_insurance_claims (id, tenant_id, customer_id, insurer_name, claim_number, claim_amount, created_by)
                  VALUES ($3, $2, 'customer-rls-test', 'RLS Test Insurance Co', 'CLM-RLS-TEST', 1000.00, 'rls-test-user')
                  RETURNING id
                )
                INSERT INTO ar_insurance_claim_supplements (id, tenant_id, claim_id, adjustment_amount, reason, created_by)
                SELECT $1, $2, claim.id, 200.00, 'RLS test supplement', 'rls-test-user' FROM claim`,
    insertParams: (id, tenant) => [id, tenant, randomUUID()],
    updateSql: `UPDATE ar_insurance_claim_supplements SET reason = 'mutated' WHERE id = $1`,
    cleanupTables: ['ar_insurance_claim_supplements', 'ar_insurance_claims'],
  });

  await testTable({
    table: 'ar_insurance_payment_applications',
    insertSql: `WITH claim AS (
                  INSERT INTO ar_insurance_claims (id, tenant_id, customer_id, insurer_name, claim_number, claim_amount, created_by)
                  VALUES ($3, $2, 'customer-rls-test', 'RLS Test Insurance Co', 'CLM-RLS-TEST', 1000.00, 'rls-test-user')
                  RETURNING id
                )
                INSERT INTO ar_insurance_payment_applications (id, tenant_id, claim_id, amount, created_by)
                SELECT $1, $2, claim.id, 700.00, 'rls-test-user' FROM claim`,
    insertParams: (id, tenant) => [id, tenant, randomUUID()],
    updateSql: `UPDATE ar_insurance_payment_applications SET amount = 999.00 WHERE id = $1`,
    cleanupTables: ['ar_insurance_payment_applications', 'ar_insurance_claims'],
  });

  await testTable({
    table: 'ar_insurance_short_pay_dispositions',
    insertSql: `WITH claim AS (
                  INSERT INTO ar_insurance_claims (id, tenant_id, customer_id, insurer_name, claim_number, claim_amount, created_by)
                  VALUES ($3, $2, 'customer-rls-test', 'RLS Test Insurance Co', 'CLM-RLS-TEST', 1000.00, 'rls-test-user')
                  RETURNING id
                )
                INSERT INTO ar_insurance_short_pay_dispositions (id, tenant_id, claim_id, amount, disposition_type, reason, created_by)
                SELECT $1, $2, claim.id, 300.00, 'WRITE_OFF', 'RLS test disposition', 'rls-test-user' FROM claim`,
    insertParams: (id, tenant) => [id, tenant, randomUUID()],
    updateSql: `UPDATE ar_insurance_short_pay_dispositions SET reason = 'mutated' WHERE id = $1`,
    cleanupTables: ['ar_insurance_short_pay_dispositions', 'ar_insurance_claims'],
  });

  await testTable({
    table: 'ar_insurance_gl_account_configs',
    insertSql: `INSERT INTO ar_insurance_gl_account_configs (tenant_id, updated_at) VALUES ($1, now())`,
    insertParams: (_id, tenant) => [tenant],
    updateSql: `UPDATE ar_insurance_gl_account_configs SET ar_insurer_control_gl_account_id = 'x' WHERE tenant_id = $1`,
    updateParam: (_idB, tenantB) => tenantB,
    cleanupTables: ['ar_insurance_gl_account_configs'],
  });
