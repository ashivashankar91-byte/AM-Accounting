    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-2 border-b">
        <button onClick={() => setTab('bank')}
          className={`px-4 py-2 text-sm font-medium border-b-2 ${tab === 'bank' ? 'border-blue-600 text-brand' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
          Bank Reconciliation
        </button>
        <button onClick={() => setTab('floor-plan')}
          className={`px-4 py-2 text-sm font-medium border-b-2 ${tab === 'floor-plan' ? 'border-blue-600 text-brand' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            ...
            <tbody>
              {(recons ?? []).length === 0 ? (
                ...
                <tr key={r.id} className="border-b border-gray-50">
                  ...
                  <td className="text-right font-mono tabular-nums">${r.glBalance.toLocaleString()}</td>
                  <td className="text-right font-mono tabular-nums">${r.bankBalance.toLocaleString()}</td>
                  <td className={`text-right font-mono tabular-nums font-medium ${Math.abs(r.variance) > 0 ? 'text-red-600' : 'text-green-600'}`}>
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    OPEN: 'bg-gray-100 text-gray-700', IN_PROGRESS: 'bg-brand-light text-brand', COMPLETED: 'bg-green-100 text-green-700',
  };
